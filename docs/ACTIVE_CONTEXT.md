...
Current milestone: STEP043 - Fix LoRa HELLO Packet Format. Fix is built; physical reciprocal gateway discovery validation is pending.

Current feature: gateway HELLO discovery over LoRa using BT-MANET JSON packets.
...
Unresolved issue: physical validation still needs to confirm `gatewayA` discovers `gatewayB`, `gatewayB` discovers `gatewayA`, and `STATUS` / `NEIGHBORS` list both gateways.
...
STEP043 trace result: RF is working because `gatewayB` receives the LoRa payload from `gatewayA`, but discovery fails when the received HELLO is not BT-MANET protocol JSON. The expected ESP32 LoRa parser path accepts JSON containing `protocolVersion`, `packetType`, `packetId`, `sourceNode`, `destinationNode`, `payload`, `hopPath`, `hopCount`, `ttl`, `previousHop`, `retryCount`, `timestamp`, `status`, and `checksum`.

STEP043 fix: `esp32-node-platformio/src/main.cpp` now sends periodic LoRa HELLO advertisements as full BT-MANET JSON from `sendHelloBroadcast()`. Relayed HELLO packets from the gateway serial path also remain JSON instead of being converted to compact `BT1|...` relay format. Non-HELLO message relay still uses the existing compact relay format. Discovery logging now includes `[HELLO] discovered <nodeId>`.

STEP043 local validation: PlatformIO builds PASS for `gatewayA_lora`, `gatewayB_lora`, `nodeA1_lora`, and `nodeA2_lora`.
...
Recommended model/tool: proceed with local ESP32/Raspberry Pi hardware validation for STEP043 reciprocal gateway HELLO discovery.
...
STEP043 physical validation procedure:
1. Flash `gatewayA_lora` and `gatewayB_lora`.
2. Monitor both serial consoles at 115200 baud.
3. Confirm LoRa TX HELLO payload begins with `{"protocolVersion":"BT-MANET-1.0","packetType":"HELLO"`.
4. Confirm Gateway B logs `[HELLO] discovered gatewayA`.
5. Confirm Gateway A logs `[HELLO] discovered gatewayB`.
6. Run `STATUS` and `NEIGHBORS` on both ESP32s.
7. Expected: neighbor table lists both gateways.

Next validation activity: physically validate STEP043 reciprocal gateway HELLO discovery, then return to gatewayB Android discovery and live discovered-node destination selection.
